...
export const updateMission = async (missionInfo, missionId) => {
    try {
        const response = await fetch(`${apiUrl}/missions/${missionId}`, {
            method: 'PUT',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ missionId: missionInfo.missionId, missionName: missionInfo.missionName, isHumanVisible: missionInfo.isHumanVisible, isZombieVisible: missionInfo.isZombieVisible, description: missionInfo.description, startTime: missionInfo.startTime, endTime: missionInfo.endTime, gameId: missionInfo.game})
        })
        if (!response.ok) {
