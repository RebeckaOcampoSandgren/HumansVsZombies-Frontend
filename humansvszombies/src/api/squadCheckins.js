...
export const updateSquadCheckin = async (squadCheckinInfo, squadCheckinId) => {
  try {
    const response = await fetch(`${apiUrl}/squadCheckins/${squadCheckinId}`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        squadCheckinId: squadCheckinInfo.squadCheckinId,
        startTime: squadCheckinInfo.startTime,
        endTime: squadCheckinInfo.endTime,
        lat: squadCheckinInfo.lat,
        lng: squadCheckinInfo.lng,
        gameId: squadCheckinInfo.gameId,
        squadId: squadCheckinInfo.squadId,
        squadMemberId: squadCheckinInfo.squadMemberId,
      }),
    });
    if (!response.ok) {
